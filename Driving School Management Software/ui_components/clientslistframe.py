class ClientsListFrame(tk.Frame):
    def __init__(self, master, show_client_profile, on_back):
        super().__init__(master)
        tk.Label(self, text='Candidats actifs', font=('Arial', 28, 'bold')).pack(pady=20)
        # Search bar
    ...
    def update_cards(self):
        ...
        try:
            ...
            if query:
                ...
                c.execute("SELECT id, name, phone, license_type, sessions_done, total_paid FROM clients WHERE checked_out=0 AND (name LIKE ? OR phone LIKE ?) ORDER BY name", (search_pattern, search_pattern))
            else:
                c.execute('SELECT id, name, phone, license_type, sessions_done, total_paid FROM clients WHERE checked_out=0 ORDER BY name')
